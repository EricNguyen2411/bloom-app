...
- **Edit / delete a memory**: each entry in the Memories gallery has Edit and Delete. Editing changes the activity text; deleting removes the entry entirely. Note: deleting does *not* subtract the growth points it already earned — the plant keeps what it grew, this just cleans up the gallery (e.g. for a duplicate or a typo you want gone).
- **Welcome screen**: the very first time the app is opened on a given phone, a short intro screen explains what it is before dropping into the empty seed. This is stored per-device (in the browser's local storage, not synced), so it'll show once on your phone and once on hers.
